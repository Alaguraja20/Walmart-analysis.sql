...
-- Median of total
select total as median from (select row_number() over (order by total asc) as rank_, total from sales) a
where rank_=(select round(count(rank_)/2) from sales);

-- Mode of total
SELECT total,count(total) freq FROM sales
group by 1
order by 2 desc;
...
SELECT AVG(quantity) AS Mean,
MAX(quantity) AS Maximum,
MIN(quantity) AS Minimum,
VARIANCE(quantity) AS Variance,
STDDEV(quantity) AS Std_deviation
FROM sales;

-- Median of quantity

select quantity as median_qty 
from (select row_number() over (order by quantity asc) as rank_qty, quantity from sales) a
where rank_qty=(select round(count(rank_qty)/2)from sales);

-- Mode of quantity

select quantity,count(quantity)as freq from sales
group by 1
order by freq desc
limit 1;
